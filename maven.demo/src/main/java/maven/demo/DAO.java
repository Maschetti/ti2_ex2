...
public class DAO {
	...
	public boolean conectar() {
		...
		String mydatabase = "aula1";
		int porta = 5432;
		String url = "jdbc:postgresql://" + serverName + ":" + porta +"/" + mydatabase;
		String username = "maschetti";
	...
	public boolean inserirCarro(Carro carro) {
		...
		try {  
			...
			st.executeUpdate("INSERT INTO Carro (marca, modelo, placa) "
	...
	public boolean excluirCarro(String id) {
		boolean status = false;
		try {  
			Statement st = conexao.createStatement();
			st.executeUpdate("DELETE FROM Carro WHERE codigo = " + id);
			st.close();
			status = true;
		} catch (SQLException u) {  
			throw new RuntimeException(u);
			...
			ResultSet rs = st.executeQuery("SELECT * FROM Carro");		
	         if(rs.next()){
	             ...
	             for(int i = 0; rs.next(); i++) {
	            	 carros[i] = new Carro(rs.getString("id"), rs.getString("marca"), 
